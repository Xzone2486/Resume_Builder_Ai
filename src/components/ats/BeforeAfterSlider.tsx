...
export function BeforeAfterSlider({ isUnlocked = false }: { isUnlocked?: boolean }) {
  ...
  const optimizedContent = (
    <div className="p-8 font-sans text-sm pointer-events-none bg-indigo-50/10 h-full">
      <h2 className="text-xl font-bold mb-4 text-indigo-900 dark:text-indigo-100">Experience</h2>
      <div className="mb-4">
        <h3 className="font-bold text-lg">Senior Frontend Engineer <span className="text-indigo-600">@ Tech Corp</span></h3>
  ...
  return (
    ...
    >
      ...
      <div 
        className="absolute inset-0 border-r-2 border-indigo-500"
      ...
      {isUnlocked && (
        ...
        >
          <div className="w-1 h-full bg-indigo-500 group-hover:w-1.5 transition-all shadow-[0_0_10px_rgba(99,102,241,0.5)]" />
          <div className="absolute w-8 h-8 bg-white border-2 border-indigo-500 rounded-full flex items-center justify-center shadow-lg transform group-hover:scale-110 transition-transform">
            <div className="w-1 h-4 border-l border-r border-indigo-300 ml-0.5" />
          </div>
        </div>
      )}
      ...
      {!isUnlocked && (
        <div className="absolute bottom-4 left-4 right-4 z-10 flex gap-4 overflow-x-auto hide-scrollbar snap-x">
          <div className="bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm border border-border p-3 rounded-lg shadow-lg flex-1 min-w-[250px] snap-center">
            <span className="text-[10px] uppercase font-bold text-indigo-500 mb-1 block">Improvement #1</span>
          ...
          <div className="bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm border border-border p-3 rounded-lg shadow-lg flex-1 min-w-[250px] snap-center">
            <span className="text-[10px] uppercase font-bold text-indigo-500 mb-1 block">Improvement #2</span>
          ...
          <div className="bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm border border-border p-3 rounded-lg shadow-lg flex-none flex items-center justify-center cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors px-6 snap-center">
            <span className="text-sm font-bold text-indigo-500 flex items-center gap-1">See all 23 <ArrowRight className="w-4 h-4" /></span>
          </div>
        </div>
      )}
    </div>
