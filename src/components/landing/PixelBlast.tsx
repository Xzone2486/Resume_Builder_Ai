...
void main(){
  ...
  if(uEnableRipples==1){
    for(int i=0;i<MAX_CLICKS;++i){
      ...
      float t=max(uTime-uClickTimes[i],0.0);float r=distance(uv,cuv);
      float waveR=speed*t;float ring=exp(-pow((r-waveR)/thickness,2.0));
  ...
  useEffect(() => {
    ...
    if (!threeRef.current) mustReinit = true;
      ...
      for (const k of needsReinitKeys)
        if (prevConfigRef.current[k] !== (cfg as any)[k]) { mustReinit = true; break; }
    }
    if (mustReinit) {
      if (threeRef.current) {
      ...
      const randomFloat = () => {
        if (typeof window !== 'undefined' && window.crypto?.getRandomValues) {
          const u32 = new Uint32Array(1); window.crypto.getRandomValues(u32); return u32[0] / 0xffffffff;
        }
        return Math.random();
      };
      ...
      if (noiseAmount > 0) {
        ...
        if (composer?.passes?.length > 0) composer.passes.forEach((p: any) => (p.renderToScreen = false));
      ...
      const animate = () => {
        ...
        if (composer) {
          ...
          composer.passes.forEach((p: any) => { const effs = p.effects; if (effs) effs.forEach((eff: any) => { const u = eff.uniforms?.get('uTime'); if (u) u.value = uniforms.uTime.value; }); });
          composer.render();
